...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('windows.update') }}" method="POST" class="max-w-lg" id="editWindowForm">
                ...
                <tbody id="columnsTableBody">
                    @foreach($columns as $column)
                        <tr class="border-b">
                            ...
                            <td class="py-2 p-3">
                            <select name="types[]" class="shadow border rounded w-full py-1 px-2 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                @foreach([
                                    'VARCHAR(255)' => 'Text', 
                                    'INT' => 'Whole Number', 
                                    'DECIMAL(10,2)' => 'Number with 2 Decimals'
                                ] as $type => $label)
                                    <option value="{{ $type }}" {{ stripos($column->Type, $type) === 0 ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <td class="py-2 p-3">
                            <form action="{{ route('windows.deleteColumn') }}" method="POST" class="max-w-lg" >
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="delete-row text-red-500 hover:text-red-700">
                                    <input type="hidden" name="column_name" value="{{ $column->Field }}"/>
        ...
        <div class="flex items-center justify-between">
            ...
            <a 
                href="{{ url('/add_window') }}" 
                class="inline-block align-baseline font-bold text-sm bg-red-500 rounded py-2 px-4 text-white hover:text-blue-800"
            >
                Cancel
            </a>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addColumnBtn.addEventListener('click', function() {
        ...
        row.innerHTML = `
            ...
            <td class="py-2 p-3">
                <select name="types[]" class="shadow border rounded w-full py-1 px-2 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    <option value="VARCHAR(255)">Text</option>
                    <option value="DECIMAL(10,2)">Number with 2 Decimals</option>
    ...
    // Convert Column Names to Snake Case Before Submitting the Form
    document.getElementById('editWindowForm').addEventListener('submit', function(e) {
        ...
        columnInputs.forEach(input => {
            input.value = input.value.replace(/\s+/g, '_').toLowerCase();  // Convert to snake_case
        });
    });
});
</script>
